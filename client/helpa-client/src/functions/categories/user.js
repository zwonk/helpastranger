...
export default {
  changeUserInfo: async function (content, captcha = null) {    let res;
    ...
    if(Object.entries(body).length > 3 && body.passw && body.new_passw && body.new_passw2)
  ...
  deleteUser: async function (captcha = null) {    let res = await api("a_users_delete", {
  ...
  membershipApply: async function(fields, captcha = null){
    ...
    dispatch(popup(null));
    dispatch(addAccountViewData({ACCOUNT_SETTINGS: {membership_applied: true}}))
